...
public class StickyNavLayout extends LinearLayout implements NestedScrollingParent{
    ...
    public void onNestedPreScroll(View target, int dx, int dy, int[] consumed){
       //hiddenTop 如果是向上 并且 上移的距离小于顶部view的高度
        boolean hiddenTop = dy > 0 && getScrollY() < mTopViewHeight;
        //showTop 如果是向下  并且target已经不能下拉 消耗掉dy
    ...
    //这个方法每次Fling只调用一次 velocityY向上为正 向下为负
    @Override
    public boolean onNestedFling(View target, float velocityX, float velocityY, boolean consumed){
        ...
        Log.i("ccccccccccccccccc","position===onNestedFling=="+velocityY);

        //consumed 判断顶部是否已经隐藏  consumed 主要的作用是用来限制下拉Fling时 限制顶部view的显示 如果大于3 下拉Fling 顶部view不显示 小于3 显示
        if (!consumed) {
            //不大于TOP_CHILD_FLING_THRESHOLD 意思是说  computeDuration这里只是根据自己的需求定义的一个动画时间 不需要纠结 返回固定值也没问题
...
//    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec){
        ...
        //viewpager的高度为总高度减去 自定义tab的高度
        params.height = getMeasuredHeight() - mNav.getMeasuredHeight();
        //设置最终的测量宽高 高为顶部view高 tab高 viewpager高
